...
| 181–182 | `IsChallengeModeActive()` macro in `constants/flags.h` wrapping `FlagGet(FLAG_DIFFICULTY_CHALLENGE)`. Used in `battle_main.c` (Set mode) and `battle_script_commands.c` (level caps). | **done (C182)** |
| 183 | Legendary Encounter Template (legend_macros.inc) — extract parameterized macros from 4 shipped legendary encounters (beasts, Ho-Oh, Groudon/Kyogre, Rayquaza). Reduces future legendary scripting from ~80 lines to ~20. Deferred since C115 (68 cycles). event_macros.inc (C179) proved the include pattern works. Schedule during v1.7 planning if legendaries are in scope. | pending |
