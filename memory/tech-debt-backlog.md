...
| 131 | Move constant validator script (scripts/check_moves.sh): scan trainer_parties.h for all MOVE_* references and verify each exists in include/constants/moves.h. Catches typos during trainer overhaul work. Has been deferred since C129 (3 cycles). Should ship no later than C132 alongside E4 implementation. Could extend existing check_trainers.sh rather than creating a new file. | pending |
| 132 | Move constant validator script (check_moves.sh): scan trainer_parties.h for all MOVE_* references and verify each exists in include/constants/moves.h. Has been deferred since C129 (4 cycles). This is the LAST cycle where it provides direct value — after C132, the trainer overhaul is complete. Either ship it this cycle by extending existing check_trainers.sh, or remove it from the backlog entirely. Deadline: C132 or drop. | pending |
