...
| 173 | Trainer Party Validation Enhancement: Extend check_trainers.sh (or create a Node.js companion script) to verify gameplay invariants — rematch party level ≥ base party level, team size monotonically increases across rematch tiers, no species duplicates within a team. Catches design regressions automatically across P2-P4 when ~50 more trainers are touched. Estimated: 30 min of cycle budget, reusable for every future trainer pass. | pending |
| 174 | Trainer Party Validation Enhancement (from C173): Extend check_trainers.sh to verify gameplay invariants — rematch party level ≥ base party level, no species duplicates within a team. Still relevant for C175-C176 ocean/cave passes. Consider building during C175 when 25 trainers make the payoff immediate. | pending |
| 175 | Trainer Party Template Script (deferred since C172): A Node.js script that converts NoItemDefaultMoves → ItemCustomMoves for a trainer ID range, generating both the party struct and trainers.h macro atomically. The 25-trainer Ocean Route pass (P3, likely C176) is the ideal payoff point. Consider building it as a warm-up step in that cycle. | pending |
| 176 | Trainer Party Template Script (deferred since C172, 4 cycles): A Node.js or shell script that takes a trainer ID and generates the correct ItemCustomMoves struct in trainer_parties.h + matching macro in trainers.h atomically. Eliminates struct-type mismatch risk. The P4 cave pass (~10 trainers in C177) is the next payoff point. Consider building as warm-up in C177 if cave trainers justify it, or drop if v1.5 completes without build failures from mismatches. | pending |
