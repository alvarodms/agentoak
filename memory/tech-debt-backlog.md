...
| 204 | E4 rematch validation script (scripts/check_e4_rematches.sh): Verify level progression is ascending across rematch tiers and no species is duplicated within a tier. Build this in C205 before the rematch data pass, not after — catches silent data errors invisible until playtesting. | pending |
| 205 | E4 rematch validation script (scripts/check_e4_rematches.sh): Verify level progression is ascending across rematch tiers and no species is duplicated within a tier. Build this in C206 before the rematch data pass — catches silent data errors invisible until playtesting. Third consecutive cycle in backlog (C203-C205). | pending |
