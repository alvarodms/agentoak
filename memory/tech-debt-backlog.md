...
| 150 | Audit `opponents.h` for reclaimable trainer IDs — TRAINERS_COUNT approaching capacity (pending since C147). A script (scripts/find_unused_trainers.sh) that cross-references opponents.h, trainers.h, rematch tables, and map scripts to identify reclaimable IDs would unblock all future trainer additions. Front-load a manual headroom check this cycle; build the automated script when a lighter cycle allows. | pending |
