...
| 181 | Define an inline helper `IsChallengeModeActive()` in a shared header (e.g., `include/difficulty.h` or in `event_data.h`) wrapping `FlagGet(FLAG_DIFFICULTY_CHALLENGE)`. Currently only one call site (battle_main.c), but Phase 4 level caps will add a second in battle_script_commands.c, and future difficulty-aware systems (trainer AI scaling, item restrictions) would add more. Small cost now, cleaner architecture for every future Challenge Mode feature. Ideal timing: C182 when the second consumer ships. | pending |
