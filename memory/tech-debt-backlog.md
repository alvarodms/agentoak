...
| 292 | Generator --force flag: Patch generate_species.cjs to accept a --force flag that deletes existing partial entries (species.h constant + species_names.h entry) before regenerating. This eliminates the recurring idempotency trap (C265, C270, C277, C288, C289) that has caused 5+ failed or wasted cycles. ~20-line change. Should be implemented if registration work recurs after v2.7. | pending |
| 293 | Battle animation templates for custom moves: all moves 355+ use the generic fallback animation. With 3 signature moves shipped and more planned, investing one cycle in 2-3 reusable animation templates (steel slash, ice punch, water blast) would make each future signature move visually distinct. ~30 lines of animation script per template, reusable across multiple moves. Not blocking but would elevate presentation significantly. | pending |
