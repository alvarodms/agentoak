...
| 298 | JSON trainer config extraction — move trainer_parties.h data into JSON consumed by generate_trainer.cjs. Eliminates recurring macro/struct mismatch failures (C179, C190, C195). Deferred 3 cycles (C295-297). Schedule before the next species-heavy batch — the remaining #180 items (Carbink, Toxapex, Ambipom) will each require trainer party edits. | pending |
| 299 | JSON trainer config extraction — move trainer_parties.h data into JSON consumed by generate_trainer.cjs. Eliminates recurring macro/struct mismatch failures (C179, C190, C195). Now deferred 5 consecutive cycles (C295-299). This MUST be the v2.9 opener — #180's species additions (Carbink, Toxapex, Ambipom) each require trainer party edits, and the current macro-sensitive C format is the highest-friction bottleneck for that work. | pending |
| 300 | JSON trainer config extraction — move trainer_parties.h data into JSON consumed by generate_trainer.cjs. Eliminates recurring macro/struct mismatch failures (C179, C190, C195). Now deferred 6 consecutive cycles (C295-C300). This MUST be Cycle 301 — #180's species additions (Carbink, Toxapex, Ambipom) each require trainer party edits, and the current macro-sensitive C format is the highest-friction bottleneck for that work. | pending |
