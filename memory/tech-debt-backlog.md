...
| 184–188 | **`difficulty_utils.inc`** macro file — `DifficultyAwareDialogue flag, text_normal, text_challenge` cuts difficulty-branching NPCs from ~15 to ~3 lines. Deferred 5 cycles (C184-188). Not needed for v1.7 atmospheric content (both modes identical). Natural fit for C190 polish phase or any future difficulty-specific variants. | pending |
...
| 189 | `difficulty_utils.inc` macro file — `DifficultyAwareDialogue flag, text_normal, text_challenge` cuts difficulty-branching NPCs from ~15 to ~3 lines. Deferred 6 cycles (C184-189). Natural fit for C190 polish phase. Not needed for C189 (both difficulty modes experience the event identically). | pending |
