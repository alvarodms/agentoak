...
| 162 | Add `make check_scripts` as a prerequisite of the main build target in the Makefile, so non-ASCII encoding errors are caught automatically before compilation. This would eliminate the single most recurring failure class (9+ cycles: 26, 64, 65, 94, 119-122, 125, 161). A 5-minute Makefile edit that pays dividends every future cycle. | pending |
