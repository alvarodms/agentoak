...
| 145 | Legendary Encounter Template (`data/scripts/legend_macros.inc`): Extract parameterized macros from 4 shipped legendary encounters (beasts, Groudon, Kyogre, Rayquaza). Reduces future legendary scripts from ~80 to ~20 lines. 24+ cycles deferred — consider scheduling if new legendary content is planned. | pending |
| 146 | Legendary Encounter Template (`data/scripts/legend_macros.inc`): Extract parameterized macros from 4 shipped legendary encounters (beasts, Groudon, Kyogre, Rayquaza). Reduces future legendary scripts from ~80 to ~20 lines. 25+ cycles deferred. Schedule when new legendary content is planned or during a lighter cycle. | pending |
| 147 | Legendary Encounter Template (`data/scripts/legend_macros.inc`): Extract parameterized macros from 4 shipped legendary encounters (beasts, Groudon, Kyogre, Rayquaza). Reduces future legendary scripts from ~80 to ~20 lines. 26+ cycles deferred. Also: audit `opponents.h` for reclaimable trainer IDs — TRAINERS_COUNT at 884, need headroom for future double battles. | pending |
| 148 | Legendary Encounter Template (`data/scripts/legend_macros.inc`): Extract parameterized macros from 4 shipped legendary encounters (beasts, Groudon, Kyogre, Rayquaza). Reduces future legendary scripts from ~80 to ~20 lines. 27+ cycles deferred. Also: audit `opponents.h` for reclaimable trainer IDs — TRAINERS_COUNT approaching capacity, need headroom for future double battles. | pending |
