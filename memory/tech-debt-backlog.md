...
| 227→236 | **RGBA auto-conversion script** (scripts/convert_sprites_indexed.cjs): Reads RGBA PNGs, extracts 16-color palette, writes indexed PNGs. Eliminates 5-10 manual actions per species cycle. Deferred 9 cycles. **Priority: HIGH — MUST ship C239 before v2.2 custom species work.** | pending — target C239 |
| 237 | RGBA auto-conversion script (scripts/convert_sprites_indexed.cjs): Reads RGBA PNGs, extracts 16-color palette, writes indexed PNGs. Now at 9 deferrals. MUST ship C239 — hard prerequisite for v2.2 custom Deoxys form work. If C237 finishes early, scaffolding the pngjs read/write loop would give C239 a head start. | pending |
