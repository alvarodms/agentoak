...
| 111 | Dialogue integrity checker script (scripts/check_dialogue.sh): grep all .inc files for smart quotes, missing $ terminators. Catches #1 recurring build failure class (4+ incidents). Being shipped THIS cycle — mark as done in tech-debt-backlog.md after implementation. | pending |
| 112 | Wire check_dialogue.sh (created C111, currently untracked) as a pre-build validation step. Even a manual `bash scripts/check_dialogue.sh` after script edits catches smart quotes and malformed .string directives before a full make — pays dividends every future script cycle. Estimated effort: 5 minutes. | pending |
