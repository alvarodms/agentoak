...
| 166 | Encounter table validation script (scripts/check_encounters.sh): After v1.4 ships 40+ rewritten tables across C165-C168, build a validator checking (1) SPECIES_* constants exist, (2) land=12/water=5/fishing=10 slot counts, (3) level ranges match the documented curve, (4) no duplicate map entries. Also: `make check_scripts` as build prerequisite (deferred 14 cycles since C152) — a 5-minute Makefile edit catching non-ASCII script errors at build time. Both should be scheduled for the first post-v1.4 engineering cycle. | pending |
| 167 | Encounter table validation script (scripts/check_encounters.sh) + `make check_scripts` as build prerequisite — now deferred 15 cycles since C152. After C168 ships the ocean routes completing v1.4, the very next cycle should be an engineering cycle addressing both. The encounter validator checks: (1) SPECIES_* constants exist in species.h, (2) land=12/water=5/fishing=10 slot counts, (3) level range consistency, (4) no duplicate map entries. The check_scripts prereq is a 5-minute Makefile edit catching non-ASCII errors at build time. Together they prevent the two most common error classes. | pending |
