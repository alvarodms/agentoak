...
| 186 | `difficulty_utils.inc` macro file — `DifficultyAwareDialogue flag, text_normal, text_challenge` reduces difficulty-branching NPCs from ~15 lines to ~3 lines. Deferred since C184. Not needed this cycle (atmospheric NPCs serve both modes equally), but natural fit for C190's polish phase when difficulty-specific NPC variants are likely. | pending |
