...
| 267→274 | **NPC dialogue template generator** (`generate_npc_dialogue.cjs`): 90+ instances of identical lock/faceplayer/msgbox/release/end + object_event + .string pattern. Trainer generator architecture (JSON→multi-file atomic writes) is directly reusable. Cuts each NPC from ~15 min to ~2 min. 8 consecutive deferrals — reclassified from trigger-based to **proactive build**. Target: **C275 refactor cycle** before regional starter arc. | pending |
