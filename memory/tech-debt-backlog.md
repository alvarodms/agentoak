...
| 115 | Legendary Encounter Template: We now have three encounter patterns (Lati roamer, beast sequence, static Ho-Oh). If v6.0 introduces more legendaries, extract a parameterized script template that takes species/level/flag/cry as variables — reduces each new legendary from ~60 lines of bespoke script to a 5-line setup + goto. Not urgent now (only one more legendary planned) but worth considering if v6.0 scope expands. Deferred since C115. | pending |
| 116 | Trainer data validation script (scripts/check_trainers.sh): cross-reference trainer IDs across opponents.h, trainers.h, and trainer_parties.h to flag missing entries or macro/struct mismatches. Pending since C106 (now 10 cycles). Should be first action of v6.0 if trainer content is planned. | pending |
