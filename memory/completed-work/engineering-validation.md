...
| `pokeemerald/scripts/generate_trainer.cjs` | **C266** | Config-driven trainer generator. JSON config → synchronized trainer_parties.h/trainers.h/opponents.h entries. Create mode (new trainer: insert define before TRAINERS_COUNT, append party, insert gTrainers[] entry). Modify mode (existing trainer: replace party block, update .party macro if type changed). Auto-detects party struct type from member fields (4 types: NoItemDefaultMoves, NoItemCustomMoves, ItemDefaultMoves, ItemCustomMoves). Normalizes missing fields (pads ITEM_NONE/MOVE_NONE). Idempotency check, --dry-run mode. Atomic writes (all-or-nothing). |
| `pokeemerald/trainer_configs/` | **C266** | Directory for trainer JSON configs. Includes test_sawyer1.json and test_roxanne1.json for round-trip validation. |
| `pokeemerald/scripts/generate_npc_dialogue.cjs` | **C275** | Config-driven NPC dialogue generator. JSON config → scripts.inc (EventScript + Text labels) + map.json (object_events entry). Charmap validation (rejects invalid escapes, smart quotes, em-dash, ASCII double-quote). MSGBOX_NPC and MSGBOX_DEFAULT script types. Single-NPC shorthand (no `npcs` array wrapper needed). `--dry-run` and `--validate` flags. Idempotency: skips if script label already exists. Atomic writes. |
