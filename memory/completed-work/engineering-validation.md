...
| `pokeemerald/scripts/generate_species.cjs` | **C254** | Config-driven species generator. JSON config → 18 files (all 19 check_species files except cry_tables.inc). Idempotency check, --dry-run mode. Anchors: before-EGG for most files, before-OLD_UNOWN for dex/mapping arrays. |
