...
static void Cmd_critcalc(void)
{
    ...
    critChance  = 2 * ((gBattleMons[gBattlerAttacker].status2 & STATUS2_FOCUS_ENERGY) != 0)
                ...
                + (gBattleMons[gBattlerAttacker].ability == ABILITY_TEMPERED_BLADE && gBattleMoves[gCurrentMove].type == TYPE_STEEL && (gBattleMoves[gCurrentMove].flags & FLAG_MAKES_CONTACT));
            ...
            else
            {
                ...
                if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP))
                {
                    ...
                    if (IsChallengeModeActive())
                    {
                        ...
                        {
                            gBattleMoveDamage = gBattleMoveDamage / 10;
                            i = STRINGID_PKMNGAINEDEXPCAPPED;
                        }
                    }
                    ...
                    if (i == STRINGID_PKMNGAINEDEXPCAPPED)
                    {
                        i = STRINGID_EMPTYSTRING4;
                        PREPARE_STRING_BUFFER(gBattleTextBuff2, i);
                        PREPARE_WORD_NUMBER_BUFFER(gBattleTextBuff3, 5, gBattleMoveDamage);
                        PrepareStringBattle(STRINGID_PKMNGAINEDEXPCAPPED, gBattleStruct->expGetterBattlerId);
                    }
                    else
                    {
